...
import { PRESET_PROVIDERS, ENDPOINT_DEFAULTS } from "../../../core/types"
import {
...
export function ManageProvidersDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const removeCustom = (name: string) => {
    updateSettings({
      ...
      disabled_providers: settings.disabled_providers.filter((n: string) => n !== name),
    })
  }
        ...
        <div className="space-y-4 mt-2 max-h-[60vh] overflow-y-auto">
          ...
          <div className="space-y-1.5">
            ...
            {PRESET_PROVIDERS.map((p) => {
              ...
              return (
                <div key={p} className="flex items-center gap-2.5 px-3 py-1.5 rounded-md bg-surface-2 border border-line-2">
                  <span className={`flex-1 text-xs ${isDisabled ? "text-ink-quaternary line-through" : "text-ink-secondary"}`}>
                    {p}
                  </span>
                  <button
          ...
          <div className="space-y-1.5">
            ...
            {customs.map((cp) => {
              ...
              return (
                <div key={cp.name} className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-surface-2 border border-line-2">
                  <span className={`text-xs ${isDisabled ? "text-ink-quaternary line-through" : "text-ink-secondary"}`}>
                    {cp.name}
                  </span>
                  <span className="text-xs text-ink-quaternary font-mono truncate flex-1">{cp.endpoint}</span>
                  <button
            ...
            {isAdding ? (
              <div className="space-y-2 p-3 rounded-md bg-surface-2 border border-accent/30">
                ...
                <div className="flex gap-2 pt-1">
                  <Button size="sm" onClick={addCustom} disabled={!newName.trim() || !newEndpoint.trim() || allNames.has(newName.trim())}>
